...
class KubernetesRunner(Runner):
    ...
    def _create_job_manifest(self, job_name: str, work_dir: str) -> Any:
        ...
        volume_mount = client.V1VolumeMount(
            name='vasp-data',
            mount_path=self.pvc_mount_path
        )
        ...
        pull_secrets = [
            ...
            for s in self.image_pull_secrets
        ] or None
        ...
        job_spec = client.V1JobSpec(
            template=client.V1PodTemplateSpec(
                metadata=client.V1ObjectMeta(
                    labels={'app': 'vasp', 'job-name': job_name}
                ),
                spec=pod_spec
            ),
    ...
    def _parse_job_status(self, job: Any, directory: str) -> JobStatus:
        ...
        if status.failed and status.failed > 0:
            return JobStatus(
                JobState.FAILED,
                jobid=job_name,
                message=self._get_failure_reason(job)
            )
    ...
    def __repr__(self) -> str:
        return (
            f"KubernetesRunner(namespace={self.namespace!r}, "
