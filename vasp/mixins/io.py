...
class IOMixin:
    ...
    def _read_vasprun_xml(self) -> None:
        ...
        try:
            ...
            # Energy
            energy_elem = calc.find('.//energy/i[@name="e_0_energy"]')
            if energy_elem is not None:
                self.results['energy'] = float(energy_elem.text)

            # Free energy
            free_energy_elem = calc.find('.//energy/i[@name="e_fr_energy"]')
            if free_energy_elem is not None:
                self.results['free_energy'] = float(free_energy_elem.text)
            ...
            if stress_elem is not None:
                ...
                voigt = np.array([
                    stress_array[0, 0], stress_array[1, 1], stress_array[2, 2],
                    stress_array[1, 2], stress_array[0, 2], stress_array[0, 1]
                ])
    ...
    def _read_outcar(self) -> None:
        ...
        # Stress
        stress_pattern = r'in kB\s+([-\d.]+)\s+([-\d.]+)\s+([-\d.]+)\s+([-\d.]+)\s+([-\d.]+)\s+([-\d.]+)'
    ...
    def read_atoms(self) -> Atoms:
        ...
        if os.path.exists(contcar) and os.path.getsize(contcar) > 0:
            ...
            return read(poscar, format='vasp')
        else:
